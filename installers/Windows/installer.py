...
INSTALL_EDITABLE_PACKAGES = os.environ.get("INSTALL_EDITABLE_PACKAGES", "").split()
...
def create_packaging_env(
    target_directory, python_version, name="packaging-env", conda_path=None
):
    """
        ...
        if name in skip_packages:
            ...
            if not releases:
                raise RuntimeError(
                    ...
                    "(bad meta-data?)".format(requirement)
                )
...
def create_pynsist_cfg(
    ...
    requirements = [
        ...
        and package_name(line) not in UNWANTED_PACKAGES
...
def run(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="installer-pynsist-") as work_dir:
            ...
            shutil.copytree(
                "installers/Windows/assets/tcl/lib", os.path.join(work_dir, "lib")
            )
            shutil.copy(
            ...
            for element in contents:
                shutil.copy(
                    os.path.join(
                        "installers/Windows/assets/nsist/plugins/x86-unicode/", element
                    ),
                    os.path.join(
                        "C:/Program Files (x86)/NSIS/Plugins/x86-unicode/", element
                    ),
                )
            ...
            if template:
                ...
                template_new_path = os.path.normpath(
                    os.path.join(work_dir, "packaging-env/Lib/site-packages/nsist")
                )
                os.makedirs(template_new_path)
                shutil.copy(
                    template, os.path.join(template_new_path, template_basename)
                )
            ...
            print("Updating/installing wheel in the virtual environment", env_python)
            subprocess_run(
                [
                    env_python,
                    "-m",
            ...
            print("Copy package .dist-info into the pynsist future " "build directory")
            package_info = about_dict(repo_root, package)
            dist_info_dir = "{}-{}.dist-info".format(
                package, package_info["__installer_version__"]
            )
            shutil.copytree(
                os.path.join(
                    work_dir, "packaging-env/Lib/site-packages", dist_info_dir
                ),
...
if __name__ == "__main__":
    ...
    parser.add_argument("python_version", help="Python version of the installer")
    ...
    parser.add_argument("icon_path", help="Path to icon to use for the installer")
    ...
    parser.add_argument("-cp", "--conda_path", help="Path to conda executable")
    parser.add_argument(
        "-s", "--suffix", help="Suffix for the name of the generated executable"
    )
    parser.add_argument(
        "-t", "--template", help="Path to .nsi template for the installer"
    )
    parser.add_argument(
        "-da",
        "--download_assets",
