...
def install():
    ...
    create_shortcut(
        python, desc, fname, '"%s"' % script, workdir, osp.join(ico_dir, "spyder.ico")
    )
    file_created(fname)
    ...
    winreg.SetValueEx(
        winreg.CreateKey(root, KEY_C1 % ("", EWS)),
        "",
        0,
        winreg.REG_SZ,
    ...
    winreg.SetValueEx(
        winreg.CreateKey(root, KEY_C1 % ("NoCon", EWS)),
        "",
        0,
        winreg.REG_SZ,
        '"%s" "%s\Scripts\spyder" "%%1"' % (pythonw, sys.prefix),
    ...
    desc = "The Scientific Python Development Environment"
    create_shortcut(
        pythonw, desc, fname, '"%s"' % script, workdir, osp.join(ico_dir, "spyder.ico")
    )
    file_created(fname)
    ...
    for key in (
        ...
        try:
            ...
            if not is_bdist_wininst:
                print(
                    "Successfully removed Spyder shortcuts from Windows "
    ...
    if not is_bdist_wininst:
        ...
        if osp.isfile(fname):
            try:
                ...
                print(
                    "Successfully removed Spyder shortcuts from your desktop.",
                    file=sys.stdout,
                )
        # clean up startmenu
        start_menu = osp.join(
        ...
        if osp.isdir(start_menu):
            for fname in os.listdir(start_menu):
                try:
                    ...
                    print(
                        "Successfully removed Spyder shortcuts from your "
                        " start menu.",
                        file=sys.stdout,
                    )
            try:
                ...
                print(
                    "Successfully removed Spyder shortcut folder from your "
                    " start menu.",
                    file=sys.stdout,
                )
