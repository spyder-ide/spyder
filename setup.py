...
def get_data_files():
    ...
    if sys.platform.startswith("linux"):
        data_files = [
            ("share/applications", ["scripts/spyder.desktop"]),
            ("share/icons", ["img_src/spyder.png"]),
            ("share/metainfo", ["scripts/org.spyder_ide.spyder.appdata.xml"]),
        ]
    elif os.name == "nt":
        data_files = [("scripts", ["img_src/spyder.ico", "img_src/spyder_reset.ico"])]
    else:
        data_files = []
...
if "dev" in __version__:
    ...
    install_requires = [
        req for req in install_requires if req.split(">")[0] not in reqs_to_loosen
    ]
...
setup_args["entry_points"] = {
    "gui_scripts": ["spyder = spyder.app.start:main"],
