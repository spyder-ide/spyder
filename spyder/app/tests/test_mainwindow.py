...
def start_new_kernel(
    startup_timeout=60, kernel_name="python", spykernel=False, **kwargs
):
...
def main_window(request, tmpdir, qtbot):
    ...
    preload_complex_project = request.node.get_closest_marker("preload_complex_project")
    ...
    if request.node.rep_setup.passed:
        if request.node.rep_call.failed:
            ...
            print(window.ipyconsole.get_current_shellwidget()._control.toPlainText())
            ...
            # this marker
            close_main_window = request.node.get_closest_marker("close_main_window")
            if close_main_window:
                ...
                try:
                    ...
                    # Close consoles
                    (window.ipyconsole.get_widget().create_new_client_if_empty) = False
                ...
                try:
                    qtbot.waitUntil(
                        lambda: (len(init_subprocesses) >= len(proc.children())),
                ...
                try:
                    ...
                    qtbot.waitUntil(
                        lambda: (len(init_files) >= len(files)), timeout=SHELL_TIMEOUT
                    )
                except Exception:
...
def test_single_instance_and_edit_magic(main_window, qtbot, tmpdir):
    ...
    lock_code = (
        ...
        "print(lock_created)".format(
            spy_dir_str=spy_dir, lock_file=get_conf_path("spyder.lock")
        )
    )
...
@pytest.mark.order(1)
@pytest.mark.skipif(
    sys.platform.startswith("linux") and not running_in_ci(),
    reason="Fails on Linux when run locally",
)
@pytest.mark.skipif(
    sys.platform == "darwin" and running_in_ci(), reason="Fails on MacOS when run in CI"
)
...
@pytest.mark.parametrize(
    "main_window", [{"spy_config": ("main", "opengl", "software")}], indirect=True
)
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    PY2 or not sys.platform == "darwin",
...
def test_get_help_ipython_console_special_characters(main_window, qtbot, tmpdir):
    ...
    qtbot.waitUntil(lambda: check_control(control, "aaʹbb"), timeout=SHELL_TIMEOUT)
    ...
    # Check that a expected text is part of the page
    qtbot.waitUntil(
        lambda: check_text(webpage, "This function docstring."), timeout=6000
    )
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="Does not work on Mac and Windows!"
)
@pytest.mark.use_introspection
...
def test_get_help_editor(main_window, qtbot, object_info):
    ...
    with qtbot.waitSignal(
        code_editor.completions_response_signal, timeout=COMPLETION_TIMEOUT
    ):
...
def test_window_title(main_window, tmpdir, qtbot):
    ...
    main_window._cli_options.window_title = "اختبار"
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="Fails sometimes on Windows and Mac"
)
...
def test_move_to_first_breakpoint(main_window, qtbot, debugcell):
    ...
    if debugcell:
        ...
        with qtbot.waitSignal(shell.executed):
            qtbot.keyClick(
                code_editor, Qt.Key_Return, modifier=Qt.AltModifier | Qt.ShiftModifier
            )
...
@pytest.mark.slow
@flaky(max_runs=3)
@pytest.mark.parametrize("test_directory", ["non_ascii_ñ_í_ç", "test_dir"])
...
def test_change_cwd_ipython_console(main_window, qtbot, tmpdir, test_directory):
    ...
    qtbot.waitUntil(
        lambda: osp.normpath(wdir.get_container().history[-1])
    ...
    qtbot.waitUntil(
        lambda: osp.normpath(treewidget.get_current_folder()) == osp.normpath(temp_dir),
        timeout=SHELL_TIMEOUT,
    )
...
@pytest.mark.skipif(
    (
        ...
        or parse_version(ipy_release.version) == parse_version("7.11.0")
    ),
...
def test_runfile_from_project_explorer(main_window, qtbot, tmpdir):
    ...
    nsb = main_window.variableexplorer.current_widget()
    qtbot.waitUntil(
        lambda: nsb.editor.source_model.rowCount() == 4, timeout=EVAL_TIMEOUT
    )
...
def test_set_new_breakpoints(main_window, qtbot):
    ...
    with qtbot.waitSignal(shell.executed):
        shell.pdb_execute("!b")
    assert (
        "1   breakpoint   keep yes   at {}:6".format(test_file) in control.toPlainText()
    )
...
def test_run_code(main_window, qtbot, tmpdir):
    ...
    qtbot.waitUntil(
        lambda: nsb.editor.source_model.rowCount() == 1, timeout=EVAL_TIMEOUT
    )
    ...
    qtbot.waitUntil(
        lambda: nsb.editor.source_model.rowCount() == 2, timeout=EVAL_TIMEOUT
    )
...
@pytest.mark.skipif(sys.platform == "darwin", reason="It fails on macOS")
@pytest.mark.parametrize(
    "main_window", [{"spy_config": ("editor", "run_cell_copy", True)}], indirect=True
)
...
def test_open_files_in_new_editor_window(main_window, qtbot):
    ...
    QTimer.singleShot(
        2000, lambda: open_file_in_editor(main_window, "script.py", directory=LOCATION)
    )
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="It only works on Linux"
)
...
@pytest.mark.skipif(
    (
        parse_version(ipy_release.version) >= parse_version("7.23.0")
        and parse_version(ipykernel.__version__) <= parse_version("5.5.3")
    ),
...
def test_tight_layout_option_for_inline_plot(main_window, qtbot, tmpdir):
    ...
    qtbot.waitUntil(lambda: "In [1]:" in control.toPlainText(), timeout=SHELL_TIMEOUT)
...
"""
    ...
    switcher_paths = [
        switcher.model.item(item_idx).get_description()
    ...
    main_window.open_switcher()
    file_b_text = switcher.model.item(switcher.model.rowCount() - 1).get_description()
...
def test_edidorstack_open_switcher_dlg(main_window, tmpdir, qtbot):
    ...
    assert editorstack.switcher_dlg.count() == len(main_window.editor.get_filenames())
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not running_in_ci() or sys.platform.startswith("linux"),
...
@pytest.mark.slow
@flaky(max_runs=3)
@pytest.mark.skipif(sys.platform == "darwin", reason="Segfaults on MacOS after passing")
def test_report_comms_error(qtbot, main_window):
    ...
    with qtbot.waitSignal(shell.executed):
        shell.execute(
            "get_ipython().kernel.frontend_comm."
    ...
    qtbot.waitUntil(
        lambda: main_window.console.error_dialog is not None, timeout=EVAL_TIMEOUT
    )
...
def test_preferences_checkboxes_not_checked_regression(main_window, qtbot):
    ...
    # Reset config
    CONF.set(
        "completions", ("provider_configuration", "lsp", "values", "pydocstyle"), False
    )

    CONF.set(
        "completions", ("provider_configuration", "lsp", "values", "pycodestyle"), False
    )
    ...
    tabs = [
        (page.tabs.widget(i).layout().itemAt(0).widget(), i)
...
def test_preferences_empty_shortcut_regression(main_window, qtbot):
    ...
    base_run_selection = CONF.get_shortcut("editor", "run selection")  # Should be F9
    ...
    qtbot.waitUntil(lambda: "ññ" in shell._control.toPlainText(), timeout=EVAL_TIMEOUT)

    assert "ññ" in shell._control.toPlainText()
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    sys.platform == "darwin" and not PY2, reason="It times out on macOS/PY3"
)
...
def test_varexp_rename(main_window, qtbot, tmpdir):
    ...
    qtbot.waitUntil(
        lambda: data(nsb.editor.model, 1, 0) == "arr2", timeout=EVAL_TIMEOUT
    )
...
@pytest.mark.skipif(sys.platform == "darwin", reason="Fails on macOS")
@pytest.mark.parametrize(
    "main_window", [{"spy_config": ("editor", "run_cell_copy", False)}], indirect=True
)
def test_runcell_edge_cases(main_window, qtbot, tmpdir):
    ...
    with qtbot.waitSignal(shell.executed):
        qtbot.keyClick(code_editor, Qt.Key_Return, modifier=Qt.ShiftModifier)
    qtbot.waitUntil(
        lambda: "runcell(0" in shell._control.toPlainText(), timeout=SHELL_TIMEOUT
    )
    ...
    with qtbot.waitSignal(shell.executed):
        qtbot.keyClick(code_editor, Qt.Key_Return, modifier=Qt.ShiftModifier)
    qtbot.waitUntil(
        lambda: "runcell(1" in shell._control.toPlainText(), timeout=SHELL_TIMEOUT
    )
...
def test_path_manager_updates_clients(qtbot, main_window, tmpdir):
    ...
    shells = [
        c.shellwidget for c in main_window.ipyconsole.get_clients() if c is not None
    ]
    assert len(shells) >= 1
    ...
    for shell in shells:
        ...
        qtbot.waitUntil(
            lambda: test_folder in control.toPlainText(), timeout=SHELL_TIMEOUT
        )
...
def test_pdb_key_leak(main_window, qtbot, tmpdir):
    ...
    try:
        ...
        qtbot.waitUntil(
            lambda: osp.normpath(str(test_file))
            in [osp.normpath(p) for p in main_window.editor.get_filenames()]
        )
        qtbot.waitUntil(
            lambda: str(test_file2)
            in [osp.normpath(p) for p in main_window.editor.get_filenames()]
        )
...
def test_pdb_step(main_window, qtbot, tmpdir, where):
    ...
    qtbot.waitUntil(
        lambda: osp.samefile(
            main_window.editor.get_current_editor().filename, str(test_file)
        )
    )
    ...
    qtbot.waitUntil(
        lambda: osp.samefile(
            main_window.editor.get_current_editor().filename, str(test_file2)
        )
    )
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not os.name == "nt", reason="Sometimes fails on Linux and hangs on Mac"
)
...
def test_running_namespace(main_window, qtbot, tmpdir):
    ...
    qtbot.waitUntil(
        lambda: "a" in nsb.editor.source_model._data
        and nsb.editor.source_model._data["a"]["view"] == "5",
        timeout=3000,
    )
...
def test_debug_namespace(main_window, qtbot, tmpdir):
    ...
    with qtbot.waitSignal(shell.executed):
        shell.execute(
            "debugfile(" + repr(str(file2)) + ", wdir=" + repr(str(tmpdir)) + ")"
        )
...
def test_run_unsaved_file_multiprocessing(main_window, qtbot):
    ...
    if os.name == "nt":
        ...
        qtbot.waitUntil(
            lambda: "None" in shell._control.toPlainText(), timeout=SHELL_TIMEOUT
        )
...
"""
    ...
    qtbot.waitUntil(
        lambda: "test" in nsb.editor.source_model._data
        and nsb.editor.source_model._data["test"]["view"] == "1",
        timeout=3000,
    )
    ...
    qtbot.waitUntil(
        lambda: "test = 2" in shell._control.toPlainText(), timeout=SHELL_TIMEOUT
    )
    ...
    qtbot.waitUntil(
        lambda: "test == 3" in shell._control.toPlainText(), timeout=SHELL_TIMEOUT
    )
    assert "test == 3" in shell._control.toPlainText()

    # Check the namespace browser is updated
    assert (
        "test" in nsb.editor.source_model._data
        and nsb.editor.source_model._data["test"]["view"] == "3"
    )
...
@flaky(max_runs=3)
@pytest.mark.parametrize(
    "main_window", [{"spy_config": ("tours", "show_tour_message", True)}], indirect=True
)
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="Flaky on Mac and Windows"
)
def test_print_comms(main_window, qtbot):
    ...
    assert "Output from spyder call 'get_namespace_view':" in control.toPlainText()
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    os.name == "nt", reason="ctypes.string_at(0) doesn't segfaults on Windows"
)
...
"""
    ...
    qtbot.waitUntil(
        lambda: "Segmentation fault" in control.toPlainText(), timeout=SHELL_TIMEOUT
    )
...
"""
    ...
    run_selection_button = main_window.run_toolbar.widgetForAction(run_selection_action)
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="Hangs sometimes on Windows and Mac"
)
