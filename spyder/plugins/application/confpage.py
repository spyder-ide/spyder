...
class ApplicationConfigPage(PluginConfigPage):
    ...
    def setup_page(self):
        ...
        language_combo = self.create_combobox(
            _("Language:"), language_choices, "interface_language", restart=True
        )

        opengl_options = ["Automatic", "Desktop", "Software", "GLES"]
        opengl_choices = list(zip(opengl_options, [c.lower() for c in opengl_options]))
        opengl_combo = self.create_combobox(
            _("Rendering engine:"), opengl_choices, "opengl", restart=True
        )
        ...
        popup_console_box = newcb(
            _("Show internal Spyder errors to report " "them to Github"),
            "show_internal_errors",
        )
        check_updates = newcb(
            _("Check for updates on startup"), "check_updates_on_startup"
        )
        ...
        margin_spin = self.create_spinbox(
            "", _("pixels"), "custom_margin", default=0, min_=0, max_=30
        )
        ...
        cursor_spin.spinbox.setEnabled(self.get_option("use_custom_cursor_blinking"))
        cursor_spin.slabel.setEnabled(self.get_option("use_custom_cursor_blinking"))
        ...
        if sys.platform == "darwin" and not running_in_mac_app():
            ...
            if als.get_bundle_identifier() is None:
                ...
                macOS_layout.addWidget(
                    QLabel(
                        _(
                            "Launch Spyder with <code>python.app</code> to enable"
        ...
        normal_radio = self.create_radiobutton(
            _("Normal"), "normal_screen_resolution", button_group=screen_resolution_bg
        )
        ...
        custom_scaling_radio = self.create_radiobutton(
            ...
            button_group=screen_resolution_bg,
            tip=_("Set this for high DPI displays when " "auto scaling does not work"),
            restart=True,
        )
        ...
        self.custom_scaling_edit = self.create_lineedit(
            ...
            tip=_(
                "Enter values for different screens "
        ...
        custom_scaling_radio.toggled.connect(self.custom_scaling_edit.setEnabled)
        ...
        if sys.platform == "darwin" and not running_in_mac_app():
            ...
            interface_tab = self.create_tab(screen_resolution_group, interface_group)

        self.tabs = QTabWidget()
        self.tabs.addTab(interface_tab, _("Interface"))
        self.tabs.addTab(self.create_tab(advanced_widget), _("Advanced settings"))
    ...
    def apply_settings(self, options):
        if "high_dpi_custom_scale_factors" in options:
            scale_factors = self.get_option("high_dpi_custom_scale_factors").split(";")
            ...
            if change_min_scale_factor:
                ...
                QMessageBox.critical(
                    ...
                    _("Error"),
                    _(
                        "We're sorry but setting a scale factor bellow 1.0 "
                ...
                self.set_option("high_dpi_custom_scale_factors", scale_factors_text)
    ...
    def _save_lang(self):
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                _("Error"),
                _(
                    "We're sorry but the following error occurred while trying "
