...
class SpyderCompletionProvider(QObject):
    """
    Spyder base class completion providers.

    All completion providers must implement this interface in order to
    interact with Spyder CodeEditor and Projects.
    """

    sig_response_ready = Signal(str, int, dict)
    """
    Use this signal to send a response back to the code completion manager.
    ...
    """
    Use this signal to indicate that the provider with `name` is ready.
    """
    ...
    def _check_interface(self):
        if self.NAME is None:
            raise SpyderAPIError(
                'A completion provider must define the `NAME` attribute!')
    ...
    def register_file(self, language, filename, codeeditor):
        ...
        raise NotImplementedError(
            'A completion provider must implement this method!')
    ...
    def status(self, value):
        if value in [ProviderStatus.Stopped, ProviderStatus.Running]:
            self._status = value
