...
class CompletionConfigPage(PluginConfigPage):
    ...
    def setup_page(self):
        ...
        for i, (provider_key, provider_name) in enumerate(self.providers):
            cb = newcb(
                _("Enable {0} provider").format(provider_name),
        ...
        completions_wait_for_ms = self.create_spinbox(
            ...
            step=10,
            tip=_(
                "Beyond this timeout the first available provider " "will be returned"
            ),
        )
        completion_hint_box = newcb(
            _("Show completion details"), "completions_hint", section="editor"
        )
        automatic_completion_box = newcb(
            _("Show completions on the fly"), "automatic_completions", section="editor"
        )
        ...
        code_snippets_box = newcb(_("Enable code snippets"), "enable_code_snippets")
        completions_after_idle = self.create_spinbox(
            ...
            max_=10000,
            step=10,
            tip=_("Default is 300 milliseconds"),
            section="editor",
        )
        completions_hint_after_idle = self.create_spinbox(
            _("Show completion details after keyboard idle (ms):"),
            None,
            "completions_hint_after_ms",
            min_=0,
            max_=10000,
            step=10,
            tip=_("Default is 500 milliseconds"),
            section="editor",
        )
        ...
        automatic_completion_box.toggled.connect(disable_completion_after_characters)
