...
from spyder.utils.syntaxhighlighters import custom_extension_lexer_mapping
...
def get_words(text, exclude_offset=None, language=""):
    ...
    tokens = [
        ...
        for x in (
            ...
            if exclude_offset is None
            or exclude_offset < m.start()
            or m.end() < exclude_offset
        )
        if x != ""
    ]
    return tokens
...
def get_parent_until(path):
    ...
    while spec:
        ...
        spec = importlib.machinery.PathFinder.find_spec("__init__", [dirname + os.sep])
