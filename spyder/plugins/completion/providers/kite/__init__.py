...
class _KiteEndpoints(type):
    ...
    HOVER_ENDPOINT = (
        "GET",
        "/api/buffer/spyder/{filename}/{hash}/hover?" "cursor_runes={cursor_runes}",
    )
    COMPLETION_ENDPOINT = ("POST", "/clientapi/editor/complete")
    SIGNATURE_ENDPOINT = ("POST", "/clientapi/editor/signatures")
    ONBOARDING_ENDPOINT = ("GET", "/clientapi/plugins/onboarding_file?editor=spyder")
    STATUS_ENDPOINT = ("GET", "/clientapi/status")  # Params: filename or filetype
