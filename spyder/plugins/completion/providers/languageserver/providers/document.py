...
class DocumentProvider:
    ...
    def document_completion_request(self, params):
        params = {
            "textDocument": {"uri": path_as_uri(params["file"])},
    ...
    def handle_completion_resolve(self, response, req_id):
        ...
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.COMPLETION_RESOLVE, {"params": response}
            )
    ...
    def process_signature_completion(self, response, req_id):
        ...
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_SIGNATURE, {"params": response}
            )
    ...
    def process_hover_result(self, result, req_id):
        ...
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_HOVER, {"params": contents}
            )
    ...
    def process_document_symbol_request(self, result, req_id):
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_SYMBOL, {"params": result}
            )
    ...
    def process_go_to_definition(self, result, req_id):
        ...
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_DEFINITION, {"params": result}
            )
    ...
    def process_folding_range(self, result, req_id):
        ...
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_FOLDING_RANGE, {"params": results}
            )
    ...
    def process_document_formatting(self, result, req_id):
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_FORMATTING, {"params": result}
            )
    ...
    def process_document_range_formatting(self, result, req_id):
        if req_id in self.req_reply:
            self.req_reply[req_id](
                CompletionRequestTypes.DOCUMENT_RANGE_FORMATTING, {"params": result}
            )
