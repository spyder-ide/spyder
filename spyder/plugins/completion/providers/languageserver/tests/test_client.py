...
from spyder.plugins.completion.api import CompletionRequestTypes, WorkspaceUpdateKind
...
def test_get_signature(lsp_client_and_completion, qtbot):
    ...
    with qtbot.waitSignal(completion.sig_response, timeout=30000) as blocker:
        client.perform_request(
            CompletionRequestTypes.DOCUMENT_SIGNATURE, signature_params
        )
...
def test_get_completions(lsp_client_and_completion, qtbot):
    ...
    with qtbot.waitSignal(completion.sig_response, timeout=30000) as blocker:
        client.perform_request(
            CompletionRequestTypes.DOCUMENT_COMPLETION, completion_params
        )
    _, response = blocker.args
...
def test_go_to_definition(lsp_client_and_completion, qtbot):
    ...
    with qtbot.waitSignal(completion.sig_response, timeout=30000) as blocker:
        client.perform_request(
            CompletionRequestTypes.DOCUMENT_DEFINITION, go_to_definition_params
        )
    _, response = blocker.args
...
def test_local_signature(lsp_client_and_completion, qtbot):
    ...
    # Perform the request
    with qtbot.waitSignal(completion.sig_response, timeout=30000) as blocker:
        client.perform_request(CompletionRequestTypes.DOCUMENT_HOVER, signature_params)
    _, response = blocker.args
...
def test_send_workspace_folders_change(lsp_client_and_completion, qtbot):
    ...
    # Test addition
    params = {"folder": folder, "kind": WorkspaceUpdateKind.ADDITION, "instance": ""}

    client.send_workspace_folders_change(params)
    assert folder in client.watched_folders

    # Test deletion
    params = {"folder": folder, "kind": WorkspaceUpdateKind.DELETION, "instance": ""}
