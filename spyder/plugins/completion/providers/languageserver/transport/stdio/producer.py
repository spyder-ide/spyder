...
class StdioLanguageServerClient(LanguageServerClient):
    ...
    def __init__(
        self, server_args="", log_file=None, zmq_in_port=7000, zmq_out_port=7001
    ):
        super(StdioLanguageServerClient, self).__init__(zmq_in_port, zmq_out_port)
        self.req_status = {}
        self.process = None

        logger.debug(repr(server_args))
        logger.debug("Environment variables: {0}".format(list(os.environ.keys())))
        ...
        self.process = popen_spawn.PopenSpawn(server_args, logfile=log_file_handle)
        ...
        self.reading_thread.initialize(
            self.process, self.zmq_out_socket, self.req_status, expectable=True
        )
