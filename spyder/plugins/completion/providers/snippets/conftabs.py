...
class SnippetsConfigTab(SpyderPreferencesTab):
    ...
    def __init__(self, parent):
        ...
        snippets_info_label = QLabel(
            _(
                "Spyder allows to define custom completion snippets to use "
            ...
            if len(errors) == 0:
                ...
                if "loading" in errors:
                    ...
                    QMessageBox.critical(
                        ...
                        _("JSON malformed"),
                        _(
                            "There was an error when trying to load the "
                            "provided JSON file: <tt>{0}</tt>"
                        ).format(errors["loading"]),
                        QMessageBox.Ok,
                    )
                    ...
                    QMessageBox.critical(
                        ...
                        _(
                            "The provided snippet file does not comply with "
                            "the Spyder JSON snippets spec and therefore it "
                            "cannot be loaded.<br><br><tt>{}</tt>"
