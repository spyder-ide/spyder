...
class CloseBracketsExtension(EditorExtension):
    ...
    def unmatched_brackets_in_line(
        self, text, closing_brackets_type=None, autoinsert=False
    ):
        """
        ...
        for pos, char in enumerate(text):
            ...
            if char in closing_brackets:
                match = self.editor.find_brace_match(
                    line_pos + pos, char, forward=False
                )
    ...
    def _autoinsert_brackets(self, char):
        ...
        if self.editor.has_selected_text():
            ...
            if (
                not trailing_text
                or trailing_text[0] in self.BRACKETS_RIGHT
                or trailing_text[0] in [",", ":", ";"]
            ):
            ...
            if (
                self.editor.next_char() == char
                and not self.editor.textCursor().atBlockEnd()
                and not self.unmatched_brackets_in_line(
