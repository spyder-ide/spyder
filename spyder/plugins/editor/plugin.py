...
from spyder.config.utils import get_edit_filetypes, get_edit_filters, get_filter
...
from spyder.plugins.editor.utils.bookmarks import load_bookmarks, save_bookmarks
from spyder.plugins.editor.utils.debugger import clear_all_breakpoints, clear_breakpoint
...
class Editor(SpyderPluginWidget, SpyderConfigurationObserver):
    ...
    # Signals
    run_in_current_ipyclient = Signal(str, str, str, bool, bool, bool, bool, bool)
    ...
    def __init__(self, parent, ignore_last_opened_files=False):
        ...
        self.editorsplitter = EditorSplitter(
            self, self, self.stack_menu_actions, first=True
        )
        ...
        if state is not None:
            self.splitter.restoreState(QByteArray().fromHex(str(state).encode("utf-8")))
    ...
    def set_outlineexplorer(self, outlineexplorer):
        ...
        self.outlineexplorer.get_widget().edit_goto.connect(
            lambda filenames, goto, word: self.load(
                filenames=filenames, goto=goto, word=word, editorwindow=self
            )
        )
        self.outlineexplorer.get_widget().edit.connect(
            lambda filenames: self.load(filenames=filenames, editorwindow=self)
        )
    ...
    def send_completion_request(self, language, request, params):
        logger.debug("Perform request {0} for: {1}".format(request, params["file"]))
        try:
    ...
    def closing_plugin(self, cancelable=False):
        ...
        self.set_option("layout_settings", self.editorsplitter.get_layout_settings())
        self.set_option(
    ...
    def get_plugin_actions(self):
        ...
        self.register_shortcut(
            self.new_action, context="Editor", name="New file", add_shortcut_to_tip=True
        )
        ...
        self.register_shortcut(
            self.open_last_closed_action, context="Editor", name="Open last closed"
        )
        ...
        save_copy_as_action = create_action(
            self,
            _("Save copy as..."),
            None,
            ima.icon("filesaveas"),
            _("Save copy of current file as..."),
            triggered=self.save_copy_as,
        )

        print_preview_action = create_action(
            self,
            _("Print preview..."),
            tip=_("Print preview..."),
            triggered=self.print_preview,
        )
        self.print_action = create_action(
            self,
            _("&Print..."),
            icon=ima.icon("print"),
        ...
        self.close_all_action = create_action(
            self,
            _("C&lose all"),
        ...
        self.register_shortcut(
            self.close_all_action, context="Editor", name="Close all"
        )
        ...
        find_next_action = create_action(
            self,
            _("Find &next"),
        ...
        self.register_shortcut(
            find_next_action, context="find_replace", name="Find next"
        )
        find_previous_action = create_action(
            self,
            _("Find &previous"),
        ...
        self.register_shortcut(
            find_previous_action, context="find_replace", name="Find previous"
        )
        _text = _("&Replace text")
        replace_action = create_action(
            self,
            _text,
        ...
        self.register_shortcut(
            replace_action, context="find_replace", name="Replace text"
        )

        # ---- Debug menu and toolbar ----
        set_clear_breakpoint_action = create_action(
            self,
        ...
        self.register_shortcut(
            set_clear_breakpoint_action, context="Editor", name="Breakpoint"
        )

        set_cond_breakpoint_action = create_action(
            self,
        ...
        self.register_shortcut(
            set_cond_breakpoint_action, context="Editor", name="Conditional breakpoint"
        )
        ...
        self.register_shortcut(
            self.debug_action, context="_", name="Debug", add_shortcut_to_tip=True
        )

        self.debug_next_action = create_action(
            self,
            _("Step"),
            icon=ima.icon("arrow-step-over"),
            tip=_("Run current line"),
            triggered=lambda: self.debug_command("next"),
        )
        self.register_shortcut(
            self.debug_next_action, "_", "Debug Step Over", add_shortcut_to_tip=True
        )
        ...
        self.register_shortcut(
            self.debug_continue_action, "_", "Debug Continue", add_shortcut_to_tip=True
        )
        ...
        self.register_shortcut(
            self.debug_step_action, "_", "Debug Step Into", add_shortcut_to_tip=True
        )
        ...
        self.register_shortcut(
            self.debug_return_action, "_", "Debug Step Return", add_shortcut_to_tip=True
        )

        self.debug_exit_action = create_action(
            self,
            _("Stop"),
            icon=ima.icon("stop_debug"),
            tip=_("Stop debugging"),
            triggered=self.stop_debugging,
        )
        self.register_shortcut(
            self.debug_exit_action, "_", "Debug Exit", add_shortcut_to_tip=True
        )
        ...
        self.register_shortcut(
            run_action, context="_", name="Run", add_shortcut_to_tip=True
        )
        ...
        self.register_shortcut(
            configure_action, context="_", name="Configure", add_shortcut_to_tip=True
        )

        re_run_action = create_action(
            self,
            _("Re-run &last script"),
        ...
        run_selected_action = create_action(
            self,
        ...
        run_to_line_action = create_action(
            self,
            _("Run &to current line"),
        ...
        run_from_line_action = create_action(
            self,
            _("Run &from current line"),
            tip=_("Run from current line"),
            triggered=self.run_from_line,
            context=Qt.WidgetShortcut,
        )
        self.register_shortcut(
            run_from_line_action,
            context="Editor",
            name="Run from line",
            add_shortcut_to_tip=True,
        )

        run_cell_action = create_action(
            self,
            _("Run cell"),
        ...
        self.register_shortcut(
            run_cell_action, context="Editor", name="Run cell", add_shortcut_to_tip=True
        )
        ...
        self.debug_cell_action = create_action(
            ...
            tip=_("Debug current cell " "(Alt+Shift+Enter)"),
        ...
        re_run_last_cell_action = create_action(
            self,
            _("Re-run last cell"),
        ...
        # --- Source code Toolbar ---
        self.todo_list_action = create_action(
            self,
            _("Show todo list"),
            icon=ima.icon("todo_list"),
            tip=_(
        ...
        self.warning_list_action = create_action(
            self,
            _("Show warning/error list"),
            icon=ima.icon("wng_list"),
        ...
        self.previous_warning_action = create_action(
            self,
            _("Previous warning/error"),
            icon=ima.icon("prev_wng"),
        ...
        self.next_warning_action = create_action(
            self,
            _("Next warning/error"),
            icon=ima.icon("next_wng"),
        ...
        self.previous_edit_cursor_action = create_action(
            self,
            _("Last edit location"),
            icon=ima.icon("last_edit_location"),
        ...
        self.previous_cursor_action = create_action(
            self,
            _("Previous cursor position"),
            icon=ima.icon("prev_cursor"),
        ...
        self.next_cursor_action = create_action(
            self,
            _("Next cursor position"),
            icon=ima.icon("next_cursor"),
        ...
        self.register_shortcut(
            self.toggle_comment_action, context="Editor", name="Toggle comment"
        )
        blockcomment_action = create_action(
            self,
            _("Add &block comment"),
        ...
        self.register_shortcut(
            blockcomment_action, context="Editor", name="Blockcomment"
        )
        unblockcomment_action = create_action(
            self,
        ...
        self.register_shortcut(
            unblockcomment_action, context="Editor", name="Unblockcomment"
        )
        ...
        self.indent_action = create_action(
            ...
            _("Indent"),
            "Tab",
            icon=ima.icon("indent"),
            tip=_("Indent current line or selection"),
            triggered=self.indent,
            context=Qt.WidgetShortcut,
        )
        self.unindent_action = create_action(
            self,
            _("Unindent"),
            "Shift+Tab",
            icon=ima.icon("unindent"),
            tip=_("Unindent current line or selection"),
            triggered=self.unindent,
            context=Qt.WidgetShortcut,
        )

        self.text_uppercase_action = create_action(
            self,
            _("Toggle Uppercase"),
            icon=ima.icon("toggle_uppercase"),
            tip=_("Change to uppercase current line or selection"),
            triggered=self.text_uppercase,
            context=Qt.WidgetShortcut,
        )
        self.register_shortcut(
            self.text_uppercase_action, context="Editor", name="transform to uppercase"
        )

        self.text_lowercase_action = create_action(
            self,
            _("Toggle Lowercase"),
            icon=ima.icon("toggle_lowercase"),
            tip=_("Change to lowercase current line or selection"),
            triggered=self.text_lowercase,
            context=Qt.WidgetShortcut,
        )
        self.register_shortcut(
            self.text_lowercase_action, context="Editor", name="transform to lowercase"
        )
        # ----------------------------------------------------------------------
        ...
        eol_actions = (self.win_eol_action, self.linux_eol_action, self.mac_eol_action)
        ...
        trailingspaces_action = create_action(
            self, _("Remove trailing spaces"), triggered=self.remove_trailing_spaces
        )

        formatter = CONF.get(
            "completions", ("provider_configuration", "lsp", "values", "formatting"), ""
        )
        self.formatting_action = create_action(
            self,
            _("Format file or selection with {0}").format(formatter.capitalize()),
        ...
        scrollpastend_action = self._create_checkable_action(
            _("Scroll past the end"), "scroll_past_end", "set_scrollpastend_enabled"
        )
        ...
        fixindentation_action = create_action(
            self,
            _("Fix indentation"),
        ...
        self.register_shortcut(gotoline_action, context="Editor", name="Go to line")

        workdir_action = create_action(
            self,
        ...
        self.clear_recent_action = create_action(
            self,
            _("Clear this list"),
        ...
        # Close
        close_actions = [self.close_action, self.close_all_action]
        ...
        self.main.run_menu_actions = run_menu_actions + self.main.run_menu_actions
        run_toolbar_actions = [
            run_action,
            run_cell_action,
            run_cell_advance_action,
        ...
        self.main.debug_menu_actions = debug_menu_actions + self.main.debug_menu_actions
    ...
    def register_plugin(self):
        ...
        outlineexplorer = self.main.get_plugin(Plugins.OutlineExplorer, error=False)
    ...
    def update_source_menu(self, options, **kwargs):
        option_names = [opt[-1] if isinstance(opt, tuple) else opt for opt in options]
    ...
    def remove_last_focused_editorstack(self, editorstack):
        for editorwindow, widget in list(self.last_focused_editorstack.items()):
    ...
    def register_editorstack(self, editorstack):
        ...
        editorstack.set_io_actions(
            self.new_action, self.open_action, self.save_action, self.revert_action
        )
        ...
        settings = (
            ...
            ("set_automatic_completions_after_ms", "automatic_completions_after_ms"),
            ("set_completions_hint_enabled", "completions_hint"),
            ("set_completions_hint_after_ms", "completions_hint_after_ms"),
        ...
        editorstack.exec_in_extconsole.connect(
            lambda text, option: self.exec_in_extconsole.emit(text, option)
        )
        editorstack.run_cell_in_ipyclient.connect(self.run_cell_in_ipyclient)
        editorstack.debug_cell_in_ipyclient.connect(self.debug_cell_in_ipyclient)
        ...
        editorstack.sig_go_to_definition.connect(
            lambda fname, line, col: self.load(fname, line, start_column=col)
        )
    ...
    @Slot(str, str, str)
    def file_saved_in_editorstack(
        self, editorstack_id_str, original_filename, filename
    ):
        ...
        if self.editorstacks is not None:
            if len(self.editorstacks) == 1:
                ...
                if editorstack is None or editorwindow is not None:
                    editorstack = self.get_last_focused_editorstack(editorwindow)
    ...
    def __load_temp_file(self):
        ...
        if not osp.isfile(self.TEMPFILE_PATH):
            ...
            text = os.linesep.join([encoding.to_unicode(qstr) for qstr in default])
            try:
    ...
    def new(self, fname=None, editorstack=None, text=None):
        ...
        finfo = self.editorstacks[0].new(fname, enc, text, default_content, empty=True)
        finfo.path = self.main.get_spyder_pythonpath()
    ...
    def update_recent_file_menu(self):
        ...
        add_actions(
            self.recent_file_menu,
            (None, self.max_recent_action, self.clear_recent_action),
        )
    ...
    def change_max_recent_files(self):
        ...
        mrf, valid = QInputDialog.getInt(
            ...
            _("Editor"),
            _("Maximum number of recent files"),
            self.get_option("max_recent_files"),
            1,
            35,
        )
        if valid:
    ...
    def load(
        ...
        if not filenames:
            ...
            if filename0 is not None:
                selectedfilter = get_filter(
                    self.edit_filetypes, osp.splitext(filename0)[1]
                )
            else:
                selectedfilter = ""
        ...
        for index, filename in enumerate(filenames):
            ...
            current_editor = self.set_current_filename(
                filename, editorwindow, focus=focus
            )
            if current_editor is None:
                ...
                finfo.path = self.main.get_spyder_pythonpath()
                self._clone_file_everywhere(finfo)
                current_editor = current_es.set_current_filename(filename, focus=focus)
    ...
    def _create_print_editor(self):
        ...
        editor.setup_editor(color_scheme="scintilla", highlight_current_line=False)
        return editor
    ...
    def print_file(self):
        ...
        # Create printer
        printer = Printer(mode=QPrinter.HighResolution, header_font=self.get_font())
    ...
    def update_cursorpos_actions(self):
        ...
        self.previous_cursor_action.setEnabled(len(self.cursor_undo_history) > 0)
        self.next_cursor_action.setEnabled(len(self.cursor_redo_history) > 0)
    ...
    def remove_file_cursor_history(self, id, filename):
        ...
        new_history = []
        for i, (cur_filename, cursor) in enumerate(self.cursor_undo_history):
            if cur_filename != filename:
                new_history.append((cur_filename, cursor))
        self.cursor_undo_history = new_history

        new_redo_history = []
        for i, (cur_filename, cursor) in enumerate(self.cursor_redo_history):
            if cur_filename != filename:
    ...
    def _pop_next_cursor_diff(self, history, current_filename, current_cursor):
        ...
        while history:
            ...
            if (
                ...
                or cursor.position() != current_cursor.position()
            ):
    ...
    def _history_steps(
        ...
        for i in range(number_steps):
            if len(forwards_history) > 0:
                ...
                backwards_history.append((current_filename, current_cursor))
                # Extract the next different cursor
                current_filename, current_cursor = self._pop_next_cursor_diff(
                    forwards_history, current_filename, current_cursor
                )
        if current_cursor is None:
            # Went too far, back up once
            current_filename, current_cursor = backwards_history.pop()
    ...
    def __move_cursor_position(self, index_move):
        ...
        self.cursor_undo_history.append((current_filename, current_cursor))
        filenames = self.get_current_editorstack().get_filenames()
        if not osp.isfile(current_filename) and current_filename not in filenames:
            self.cursor_undo_history.pop()
        else:
    ...
    def clear_breakpoint(self, filename, lineno):
        ...
        if editorstack is not None:
            ...
            if index is not None:
                editorstack.data[index].editor.debugger.toogle_breakpoint(lineno)
    ...
    def handle_run_cell(self, cell_name, filename):
        ...
        if editor is None:
            raise RuntimeError("File {} not open in the editor".format(filename))
    ...
    def handle_get_file_code(self, filename, save_all=True):
        ...
        """
        editorstack = self._get_editorstack()
        if save_all and CONF.get("editor", "save_all_before_run", default=True):
    ...
    def run_file(self, debug=False):
        ...
        if runconf is None:
            ...
            if self.dialog_size is not None:
                dialog.resize(self.dialog_size)
            dialog.setup(fname)
            if CONF.get("run", "open_at_least_once", not running_under_pytest()):
        ...
        if self.editorstacks is not None:
            ...
            for editorstack in self.editorstacks:
                if currentline_n in options:
                    editorstack.set_highlight_current_line_enabled(currentline_o)
                if currentcell_n in options:
                    editorstack.set_highlight_current_cell_enabled(currentcell_o)
            ...
            for editorstack in self.editorstacks:
                ...
                if linenb_n in options:
                    editorstack.set_linenumbers_enabled(linenb_o, current_finfo=finfo)
                ...
                if removetrail_newlines_n in options:
                    editorstack.set_remove_trailing_newlines(removetrail_newlines_o)
                ...
                if todo_n in options:
                    editorstack.set_todolist_enabled(todo_o, current_finfo=finfo)
    ...
    @on_conf_change(
        option=("provider_configuration", "lsp", "values", "enable_hover_hints"),
        section="completions",
    )
            ...
            if layout:
                ...
                if cfname in filenames and len(filenames) == len(clines):
                    ...
                    if self.autosave.recover_files_to_open:
                        self.load(
                            self.autosave.recover_files_to_open,
                            set_focus=False,
                            add_where="end",
                        )
                else:
    ...
    def register_panel(
        self, panel_class, *args, position=Panel.Position.LEFT, **kwargs
    ):
        """Register a panel in all the editorstacks in the given position."""
        for editorstack in self.editorstacks:
            editorstack.register_panel(panel_class, *args, position=position, **kwargs)
