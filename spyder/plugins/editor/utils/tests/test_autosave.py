...
def test_get_files_to_recover_with_empty_autosave_dir(mocker, tmpdir):
    ...
    mocker.patch(
        "spyder.plugins.editor.utils.autosave.get_conf_path", return_value=str(tmpdir)
    )
    addon = AutosaveForPlugin(None)
...
@pytest.mark.parametrize(
    "running,empty", [(True, False), (False, False), (False, True)]
)
def test_get_files_to_recover_with_one_pid_file(mocker, tmpdir, running, empty):
    ...
    mock_is_spyder_process = mocker.patch(
        "spyder.plugins.editor.utils.autosave.is_spyder_process", return_value=running
    )
...
def test_get_files_to_recover_without_autosave_dir(mocker):
    ...
    directory."""
    mocker.patch(
        "spyder.plugins.editor.utils.autosave.get_conf_path",
...
@pytest.mark.parametrize(
    "in_mapping,on_disk", [(False, False), (True, False), (False, True)]
)
...
def test_autosave(mocker, have_hash):
    ...
    mock_fileinfo = mocker.Mock(
        editor=mock_editor, filename="orig", newly_created=False
    )
...
def test_autosave_file_renamed(mocker, tmpdir, have_hash):
    ...
    mock_editor = mocker.Mock()
    mock_fileinfo = mocker.Mock(
        editor=mock_editor, filename="new_foo.py", newly_created=False
    )
    mock_document = mocker.Mock()
    mock_fileinfo.editor.document.return_value = mock_document
    mock_stack = mocker.Mock(data=[mock_fileinfo])
