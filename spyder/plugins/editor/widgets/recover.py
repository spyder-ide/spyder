    ...
    def file_data_to_str(self, data):
        ...
        try:
            mtime_as_str = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(data["mtime"])
            )
    ...
    def restore(self, idx):
        ...
        if orig:
            ...
            orig_name, ignored = getsavefilename(
                self, _("Restore autosave file to ..."), osp.basename(autosave["name"])
            )
            if not orig_name:
                return
        try:
            ...
            text = _("Unable to restore {} using {}").format(
                orig_name, autosave["name"]
            )
