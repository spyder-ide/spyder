...
def test_find_number_matches(setup_editor):
    ...
    editor_stack.find_widget.find(
        changed=False, forward=True, rehighlight=False, multiline_replace_check=False
    )
...
def test_copy_lines_down_up(editor_bot, mocker, qtbot):
    ...
    # message box.
    mocker.patch("spyder.plugins.editor.widgets.editor.osp.isfile", returned_value=True)
...
def test_get_current_word(base_editor_bot, qtbot):
    ...
    editor_stack = base_editor_bot
    text = (
        "some words with non-ascii  characters\n"
        "niño\n"
        "garçon\n"
        "α alpha greek\n"
...
def test_opening_sets_file_hash(base_editor_bot, mocker):
    ...
    editor_stack = base_editor_bot
    mocker.patch(
        "spyder.plugins.editor.widgets.editor.encoding.read",
...
def test_maybe_autosave_does_not_save_after_open(
    base_editor_bot, mocker, qtbot, filename
):
    """
