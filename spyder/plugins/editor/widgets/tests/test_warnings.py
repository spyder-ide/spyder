...
TEXT = (
    ...
    "    return a\n"
    "undefined_function()"
)  # Undefined name 'undefined_function'
...
def completions_codeeditor_linting(request, qtbot, completions_codeeditor):
    ...
    CONF.set(
        "completions", ("provider_configuration", "lsp", "values", "pydocstyle"), True
    )

    CONF.set(
        "completions", ("provider_configuration", "lsp", "values", "pycodestyle"), True
    )
...
def test_ignore_warnings_with_comments(
    ...
    """
    editor, _ = completions_codeeditor_linting
    editor.textCursor().insertText(f"foo {ignore_comment}\n" "bar\n")
