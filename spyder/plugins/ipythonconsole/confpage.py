...
class IPythonConsoleConfigPage(PluginConfigPage):
    def setup_page(self):
        ...
        banner_box = newcb(
            ...
            "show_banner",
            tip=_(
                "This option lets you hide the message "
                "shown at\nthe top of the console when "
                "it's opened."
            ),
        )
        calltips_box = newcb(_("Show calltips"), "show_calltips")
        ask_box = newcb(_("Ask for confirmation before closing"), "ask_before_closing")
        reset_namespace_box = newcb(
            _("Ask for confirmation before removing all user-defined " "variables"),
        ...
        bend_label = QLabel(
            _(
                "Decide how graphics are going to be displayed "
        ...
        backend_box = self.create_combobox(
            ...
            "pylab/backend",
            default=0,
            tip=_("This option will be applied the next time a console is " "opened."),
        )
        ...
        inline_label = QLabel(
            _("Decide how to render the figures created by " "this backend")
        )
        inline_label.setWordWrap(True)
        formats = (("PNG", 0), ("SVG", 1))
        format_box = self.create_combobox(
            _("Format:") + "   ", formats, "pylab/inline/figure_format", default=0
        )
        resolution_spin = self.create_spinbox(
            ...
            max_=999,
            step=0.1,
            tip=_("Only used when the format is PNG. Default is " "72"),
        )
        ...
        run_lines_label = QLabel(
            _(
                "You can run several lines of code when "
        ...
        run_lines_edit = self.create_lineedit(
            _("Lines:"), "startup/run_lines", "", alignment=Qt.Horizontal
        )
        ...
        run_file_label = QLabel(
            _(
                "You can also run a whole file at startup "
                "instead of just some lines (This is "
                "similar to have a PYTHONSTARTUP file)."
            )
        )
        run_file_label.setWordWrap(True)
        file_radio = newcb(_("Use the following file:"), "startup/use_run_file", False)
        ...
        prevent_closing_box = newcb(
            ...
            "pdb_prevent_closing",
            tip=_(
                "This option prevents the user from closing a file while"
                " it is debugged."
            ),
        )
        ...
        continue_box = newcb(
            ...
            "pdb_stop_first_line",
            tip=_(
                "This option lets you decide if the debugger should"
        ...
        exclamation_mark_box = newcb(
            ...
            tip=_(
                "This option lets you decide if the Pdb commands should "
        ...
        jedi_label = QLabel(
            _(
                "Enable Jedi-based <tt>Tab</tt> completion "
        ...
        jedi_box = newcb(
            _("Use Jedi completion in the IPython console"),
            "jedi_completer",
            tip=_(
                "<b>Warning</b>: "
        ...
        greedy_label = QLabel(
            _(
                "Enable <tt>Tab</tt> completion on elements "
        ...
        greedy_box = newcb(
            _("Use greedy completion in the IPython console"),
            "greedy_completer",
        ...
        autocall_label = QLabel(
            _(
                "Autocall makes IPython automatically call "
        ...
        autocall_box = self.create_combobox(
            ...
            default=0,
            tip=_(
                "On <b>%s</b> mode, Autocall is not applied if "
        ...
        sympy_label = QLabel(
            _(
                "Perfom symbolic operations in the console "
        ...
        prompts_label = QLabel(
            _("Modify how Input and Output prompts are " "shown in the console.")
        )
        ...
        hide_cmd_windows = newcb(
            _(
                "Hide command line output windows "
        ...
        self.tabs.addTab(
            self.create_tab(pylab_group, backend_group, inline_group), _("Graphics")
        )
        self.tabs.addTab(self.create_tab(run_lines_group, run_file_group), _("Startup"))
        self.tabs.addTab(
            self.create_tab(debug_group, pdb_run_lines_group), _("Debugger")
        )
        self.tabs.addTab(
            self.create_tab(
                jedi_group,
