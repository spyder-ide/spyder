...
NON_ASCII_DIR = osp.join(TEMP_DIRECTORY, "測試", "اختبار")
...
def ipyconsole(qtbot, request, tmpdir):
    ...
    if use_startup_wdir:
        ...
        configuration.set("workingdir", "startup/use_project_or_home_directory", False)
        ...
        configuration.set("workingdir", "startup/use_project_or_home_directory", True)
    ...
    if test_environment_interpreter:
        configuration.set("main_interpreter", "default", False)
        configuration.set("main_interpreter", "executable", get_conda_test_env())
    else:
        configuration.set("main_interpreter", "default", True)
        configuration.set("main_interpreter", "executable", "")
    ...
    console._register()
    console.create_new_client(is_pylab=is_pylab, is_sympy=is_sympy, is_cython=is_cython)
    ...
    try:
        ...
        qtbot.waitUntil(
            lambda: (len(init_subprocesses) - 1 >= len(proc.children())),
    ...
    try:
        qtbot.waitUntil(
            lambda: (len(init_files) >= len(proc.open_files())), timeout=SHELL_TIMEOUT
        )
    except Exception:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("absolute", ["x", "/", "out"], ["Parameters<br>", "x : array_like ..."]),
    ],
)
@pytest.mark.skipif(not os.name == "nt", reason="Times out on macOS and fails on Linux")
...
@pytest.mark.skipif(
    running_in_ci() and not os.name == "nt", reason="Times out on Linux and macOS"
)
...
@pytest.mark.xfail(
    parse_version("1.0") < parse_version(sympy.__version__) < parse_version("1.2"),
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    running_in_ci() and sys.platform == "darwin", reason="Hangs sometimes on macOS"
)
@pytest.mark.skipif(
    os.name == "nt" and running_in_ci_with_conda(),
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    PY2 or IPython.version_info < (7, 17), reason="insert is not the same in py2"
)
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    os.name == "nt" or (PY2 and PYQT5), reason="It times out frequently"
)
...
def test_set_elapsed_time(ipyconsole, qtbot):
    ...
    assert (
        "00:02:00" in main_widget.time_label.text()
        or "00:02:01" in main_widget.time_label.text()
    )
    ...
    assert (
        ...
        or "00:02:02" in main_widget.time_label.text()
    )
...
def test_stderr_file_is_removed_one_kernel(ipyconsole, qtbot, monkeypatch):
    ...
    monkeypatch.setattr(
        QMessageBox, "question", classmethod(lambda *args: QMessageBox.Yes)
    )
...
def test_stderr_file_remains_two_kernels(ipyconsole, qtbot, monkeypatch):
    ...
    # In a normal situation file should exist
    monkeypatch.setattr(
        QMessageBox, "question", classmethod(lambda *args: QMessageBox.No)
    )
    assert osp.exists(client.stderr_obj.filename)
    ipyconsole.close_client(client=client)
    assert osp.exists(client.stderr_obj.filename)
...
def test_kernel_crash(ipyconsole, qtbot):
    ...
    ipy_kernel_cfg = osp.join(
        get_ipython_dir(), "profile_default", "ipython_kernel_config.py"
    )
    try:
        ...
        qtbot.waitUntil(
            lambda: ipyconsole.get_clients()[-1].is_error_shown, timeout=6000
        )
        ...
        qtbot.waitUntil(
            lambda: check_text(webpage, "Bad config encountered"), timeout=6000
        )
    finally:
...
@flaky(max_runs=3)
@pytest.mark.skipif(
    not sys.platform.startswith("linux") or PY2,
...
def test_kernel_kill(ipyconsole, qtbot):
    ...
    qtbot.waitUntil(
        lambda: shell.spyder_kernel_comm._comms[new_open_comms[0]]["status"] == "ready"
    )
...
def test_wrong_std_module(ipyconsole, qtbot, tmpdir, spyder_pythonpath):
    ...
    if spyder_pythonpath:
        check_sys_path = "import sys; path_added = r'{}' in sys.path".format(
            str(tmpdir)
        )
        with qtbot.waitSignal(shell.sig_prompt_ready, timeout=30000):
...
def test_kernel_restart_after_manual_restart_and_crash(ipyconsole, qtbot):
    ...
    qtbot.waitUntil(
        lambda: shell.spyder_kernel_comm._comms[open_comms[0]]["status"] == "ready"
    )
...
def test_stderr_poll(ipyconsole, qtbot):
    ...
    # Wait for the poll
    qtbot.waitUntil(
        lambda: "test_test" in ipyconsole.get_widget().get_focus_widget().toPlainText()
    )
    ...
    assert (
        ipyconsole.get_widget().get_focus_widget().toPlainText().count("test_test") == 2
    )
...
def test_recursive_pdb(ipyconsole, qtbot):
    ...
    qtbot.keyClick(control, Qt.Key_Tab)
    qtbot.waitUntil(
        lambda: control.toPlainText().split()[-1] == "abab", timeout=SHELL_TIMEOUT
    )
    # quit one layer
    with qtbot.waitSignal(shell.executed):
        shell.pdb_execute("!quit")
...
def test_pdb_comprehension_namespace(ipyconsole, qtbot, tmpdir):
    ...
    shell.call_kernel(interrupt=True).set_namespace_view_settings(settings)
...
def test_startup_run_lines_project_directory(ipyconsole, qtbot, tmpdir):
    ...
    ipyconsole.set_conf("spyder_pythonpath", [project_dir], section="main")

    # Config console with project path
    ipyconsole.set_conf(
        "startup/run_lines", "from project_script import *", section="ipython_console"
    )
    ipyconsole.set_conf(
        "console/use_project_or_home_directory",
        True,
