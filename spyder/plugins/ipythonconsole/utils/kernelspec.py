...
class SpyderKernelSpec(KernelSpec, SpyderConfigurationAccessor):
    ...
    def env(self, env_vars):
        ...
        env_vars.update({
            # Note: We can't remove this variable because it's required by
            # spydercustomize, which runs before comms are opened. So, it can't
            # be made a kernel config option.
            'SPY_HIDE_CMD': self.get_conf('hide_cmd_windows'),
            # Note: The next two variables are only needed for local kernels
            # and can't be changed by users. So, there's no need to send them
            # as kernel config options.
