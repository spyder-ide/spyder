...
class DebuggingHistoryWidget(RichJupyterWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self._pdb_history = [
                line[-1]
                for line in self._pdb_history_file.get_tail(
    ...
    def add_to_pdb_history(self, line):
        ...
        cmd = line.split(" ")[0]
        args = line.split(" ")[1:]
        is_pdb_cmd = cmd.strip() and cmd[0] != "!" and "do_" + cmd in dir(pdb.Pdb)
...
class DebuggingWidget(DebuggingHistoryWidget, SpyderConfigurationAccessor):
    ...
    def _pdb_cmd_prefix(self):
        ...
        prefix = ""
        if self.spyder_kernel_comm.is_open() and self.is_pdb_using_exclamantion_mark():
            prefix = "!"
        return prefix
    ...
    def _handle_input_request(self, msg):
        ...
        if (
            not self.spyder_kernel_comm.is_open()
            and msg["content"]["prompt"] == "ipdb> "
        ):
    ...
    def pdb_execute(self, line, hidden=False, echo_stack_entry=True, add_history=True):
        """
        ...
        if hidden:
            ...
            self.sig_pdb_state_changed.emit(False, self.get_pdb_last_step())
        ...
        self._pdb_input_queue.append((line, hidden, echo_stack_entry, add_history))

    def get_pdb_settings(self):
        """Get pdb settings"""
        return {
            "breakpoints": self.get_conf("breakpoints", default={}, section="run"),
    ...
    def stop_debugging(self):
        """Stop debugging."""
        if self.spyder_kernel_comm.is_open() and not self.is_waiting_pdb_input():
    ...
    def set_pdb_ignore_lib(self, pdb_ignore_lib):
        ...
        self.call_kernel(interrupt=True).set_pdb_ignore_lib(pdb_ignore_lib)

    def set_pdb_execute_events(self, pdb_execute_events):
        """Set pdb_execute_events into a debugging session"""
        self.call_kernel(interrupt=True).set_pdb_execute_events(pdb_execute_events)
    ...
    def show_pdb_output(self, text):
        ...
        self._append_html(
            '<span class="out-prompt">%s</span>' % prompt, before_prompt=True
        )
    ...
    def get_pdb_last_step(self):
        ...
        if fname is None:
            return {}
        return {"fname": fname, "lineno": lineno}
    ...
    def reset(self, clear=False):
        ...
        if clear and self.is_waiting_pdb_input():
            ...
            self._reading = False
            self._readline(
                prompt=prompt, callback=self._pdb_readline_callback, password=password
            )
    ...
    def _handle_kernel_info_reply(self, rep):
        ...
        pygments_lexer = rep["content"]["language_info"].get("pygments_lexer", "")
        try:
            # add custom lexer
    ...
    def _update_pdb_prompt(self, prompt, password=None):
        ...
        self._set_continuation_prompt(self._make_continuation_prompt(prompt), html=True)
    ...
    def execute(self, source=None, hidden=False, interactive=False):
        ...
        return super(DebuggingWidget, self).execute(source, hidden, interactive)
    ...
    def pdb_input(self, prompt, password=None):
        ...
        print_prompt = not self._reading and (
            len(self._pdb_input_queue) == 0 or not self._pdb_input_queue[0][1]
        )
    ...
    def _show_prompt(self, prompt=None, html=False, newline=True, separator=True):
        ...
        super(DebuggingWidget, self)._show_prompt(prompt, html, newline, separator)
    ...
    def _event_filter_console_keypress(self, event):
        ...
        if self.is_waiting_pdb_input():
            ...
            self._reading = False
            try:
                ret = super(DebuggingWidget, self)._event_filter_console_keypress(event)
                return ret
            finally:
                self._reading = self._tmp_reading
        else:
            return super(DebuggingWidget, self)._event_filter_console_keypress(event)
