...
from spyder.plugins.languageserver.api import ClientConstants
...
class LSPClient(QObject, LSPMethodProviderMixIn):
    ...
    sig_initialized = Signal(dict, str)
    """
    Signal to inform the editor plugin that the client has started properly
                    ...
                    if self.language == 'python':
                        ...
                        if get_debug_level() > 0 or DEV:
                            ...
                            if traceback is not None:
                                ...
                                traceback = traceback + '\n' + message
                                self.sig_server_errored.emit(traceback)
                        req_id = resp['id']
    ...
    def process_server_capabilities(self, server_capabilites, *args):
        ...
        self.sig_initialized.emit(self.server_capabilites, self.language)
