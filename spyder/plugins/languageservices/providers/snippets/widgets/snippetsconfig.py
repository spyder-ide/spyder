...
# Languages snippets can be defined for (every language Spyder knows)
SUPPORTED_LANGUAGES_PY = sorted(
    (language.name for language in Language), key=str.lower
)
LANGUAGE_NAMES = {name.lower(): name for name in SUPPORTED_LANGUAGES_PY}
LANGUAGE_SET = set(LANGUAGE_NAMES)
PYTHON_POS = SUPPORTED_LANGUAGES_PY.index("Python")
