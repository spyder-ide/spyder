        ...
        def docclass(self, object, name=None, mod=None, funcs={}, classes={}, *ignored):
            ...
            while attrs:
                ...
                attrs = spilldescriptors(
                    "Data descriptors %s" % tag,
                    attrs,
                    lambda t: t[1] == "data descriptor",
                )
                attrs = spilldata(
                    "Data and other attributes %s" % tag,
                    attrs,
                    lambda t: t[1] == "data",
                )
                assert attrs == []
...
def _url_handler(url, content_type="text/html"):
    ...
    class _HTMLDoc(CustomHTMLDoc):
        def page(self, title, contents):
            ...
            code_style = '<style>code {font-family: "%s"}</style>' % plain_text_font
...
%s<div style="clear:both;padding-top:.7em;">%s</div>
    ...
    def html_getfile(path):
        ...
        contents = heading + html.bigsection(
            "File: {}".format(path), body, css_class="getfile"
        )
    ...
    def html_keywords():
        ...
        contents = (
            heading
            + "<br>"
            + html.bigsection("Keywords", contents, css_class="keywords")
        )
    ...
    def html_topicpage(topic):
        ...
        if xrefs:
            ...
            xrefs = html.html_section(
                "Related help topics: ", xrefs, css_class="topics"
            )
    ...
    def html_getobj(url):
        ...
        if obj is None and url != "None":
            raise ValueError(
                _(
                    "There was an error while retrieving documentation "
            ...
            if url in ("", "index"):
                ...
                if op == "search?key":
                    ...
                    raise ValueError(
                        _(
                            "There was an error while retrieving documentation "
                            "for the object you requested: Bad URL %s"
                        )
                        % url
                    )
            else:
