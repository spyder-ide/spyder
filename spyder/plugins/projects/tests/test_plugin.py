...
def projects(qtbot, mocker, request, tmpdir):
    ...
    mocker.patch.object(
        spyder.plugins.base.SpyderDockWidget, "install_tab_event_filter"
    )
    yield projects
...
def test_set_get_project_filenames_when_closing_no_files(create_projects, tmpdir):
    """
...
def test_project_explorer_tree_root(projects, tmpdir, qtbot):
    ...
    for ppath in [ppath1, ppath2]:
        ...
        assert projects.get_widget().treewidget.rootIndex().data() == osp.basename(
            osp.dirname(ppath)
        )
...
def test_filesystem_notifications(qtbot, projects, tmpdir):
    ...
    if not sys.platform == "darwin":
        ...
        with qtbot.waitSignal(fs_handler.sig_file_modified, timeout=3000) as blocker:
            file3.write("abc")
...
def test_loaded_and_closed_signals(create_projects, tmpdir, mocker, qtbot):
    ...
    # Switch to another project.
    with qtbot.waitSignals([projects.sig_project_loaded, projects.sig_project_closed]):
        projects.open_project(path=path2)
