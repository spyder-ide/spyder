...
class RemoteClient(SpyderPluginV2):
    ...
    NAME = "remoteclient"
    OPTIONAL = [Plugins.MainMenu, OptionalPlugins.EnvsManager]
    CONF_SECTION = NAME
    ...
    @on_plugin_available(plugin=OptionalPlugins.EnvsManager)
    def on_envsmanager_available(self):
        envsmanager = self.get_plugin(OptionalPlugins.EnvsManager)
        envsmanager.sig_set_default_kernel_spec_requested.connect(
            self.set_default_kernel_spec
        )

    @on_plugin_teardown(plugin=OptionalPlugins.EnvsManager)
    def on_envsmanager_teardown(self):
        envsmanager = self.get_plugin(OptionalPlugins.EnvsManager)
        envsmanager.sig_set_default_kernel_spec_requested.disconnect(
            self.set_default_kernel_spec
        )
