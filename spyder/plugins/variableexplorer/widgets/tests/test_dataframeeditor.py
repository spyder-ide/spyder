...
def test_dataframe_multiindex():
    ...
    arrays = [
        numpy.array(["bar", "bar", "baz", "baz", "foo", "foo", "qux", "qux"]),
...
@pytest.mark.skipif(
    is_module_installed("pandas", "<0.19"), reason="It doesn't work for Pandas 0.19-"
)
...
def test_dataframemodel_set_data_overflow(monkeypatch):
    ...
    attr_to_patch = (
        "spyder.plugins.variableexplorer.widgets" + ".dataframeeditor.QMessageBox"
    )
