...
class IconManager:
    ...
    def _get_qtaargs(self):
        MAIN_FG_COLOR = self.foreground_color
    ...
    def icon(self, name, scale_factor=None, resample=False, icon_path=None):
        """"""
        ...
        if theme == 'spyder 3':
            ...
            args, kwargs = self._get_qtaargs()[name]
            if scale_factor is not None:
                kwargs['scale_factor'] = scale_factor
            qicon = qta.icon(*args, **kwargs)
        elif theme == 'spyder 2':
            ...
            qicon = icon if icon is not None else QIcon()
    ...
    def get_icon_by_extension_or_type(self, fname, scale_factor):
        ...
        application_icons = {}
        application_icons.update(self.BIN_FILE_ICONS)
        application_icons.update(self.DOCUMENT_FILE_ICONS)

        if osp.isdir(fname):
            return self.icon('DirOpenIcon', scale_factor)
        else:
            ...
            if extension in self.OFFICE_FILE_ICONS:
                icon_by_extension = self.icon(
                    self.OFFICE_FILE_ICONS[extension], scale_factor)
                ...
                if extension == '.ipynb':
                    ...
                    icon_by_extension = self.icon('TextFileIcon',
                                                  scale_factor)
