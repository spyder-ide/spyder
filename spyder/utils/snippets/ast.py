...
GRAMMAR, _, _, PARSE_TABLE = create_LL1_parsing_table(starting_rule=STARTING_RULE)


CONTEXT_SWITCHER = {
    "TEXT": {"default": nodes.TextNode, "args": True},
    "TEXT_SNIPPETS": {"default": nodes.TextNode, "args": True},
    "TEXT_NO_COL": {"default": nodes.TextNode, "args": True},
    "TEXT_COL": {"default": nodes.TextNode, "args": True},
    "TEXT_REGEX": {"default": nodes.TextNode, "args": True},
    "TEXT_FORMAT": {"default": nodes.TextNode, "args": True},
    "EXPRLIST": {"default": nodes.TextNode, "args": True},
    "SNIPPET": {"default": None, "args": True},
    "INTSNIPPET": {"SNIPPET": nodes.TabstopSnippetNode, "args": False},
    "VARSNIPPET": {"SNIPPET": nodes.VariableSnippetNode, "args": False},
    "COLONBODY": {
        "INTSNIPPET": nodes.PlaceholderNode,
        "VARSNIPPET": nodes.VariablePlaceholderNode,
        "args": False,
    },
    "PIPEBODY": {"INTSNIPPET": nodes.ChoiceNode, "args": False},
}
    ...
    if current_rule in CONTEXT_SWITCHER:
        ...
        if current_ctx_name in rule_switch:
            ...
            raise ValueError(
                ...
                "context {0} from {1}".format(current_rule, current_ctx_name)
            )
        ...
        if rule_switch["args"]:
            ...
            new_args = []
            new_prefix = []
    return (new_ctx, new_args, new_prefix, context_stack, args_stack, prefix_stack)
    ...
    while len(stack) > 0:
        ...
        if current_rule in GRAMMAR:
            ...
            if peek_token.token in follow_predictions:
                ...
                raise SyntaxError(
                    "Syntax Error: Expected any of the following"
                    " characters: {0}, got {1}".format(
                        list(follow_predictions.keys()), peek_token
                    )
                )
            current_prefix.pop(0)
            stack = next_productions + stack
            new_ctx = switch_context(
                current_rule,
                current_ctx,
                current_args,
                current_prefix,
            ...
            if peek_token.token == current_rule:
                ...
                raise SyntaxError(
                    "Syntax Error: Expected {0}, got {1}".format(
            ...
            if current_name in IGNORE_TERMINALS:
                add_to_args = peek_token.token not in IGNORE_TERMINALS[current_name]
