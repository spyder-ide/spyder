...
def get_search_score(query, choice, ignore_case=True, apply_regex=True, template="{}"):
    ...
    if apply_regex:
        ...
        sep = "-"  # Matches will be replaced by this character
        let = "x"  # Nonmatches (except spaed) will be replaced by this
        score = 0

        exact_words = [
            query == to_text_string(word, encoding="utf-8")
            for word in choice.split(" ")
        ]
        partial_words = [query in word for word in choice.split(" ")]
...
def get_search_scores(
    query, choices, ignore_case=True, template="{}", valid_only=False, sort=False
):
