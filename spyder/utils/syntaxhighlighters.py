...
from spyder.py3compat import builtins, is_text_string, to_text_string, PY3, PY36_OR_MORE
...
DEFAULT_PATTERNS = {
    ...
    "mail": r"(?:mailto:\s*)?([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})",
...
class BaseSH(QSyntaxHighlighter):
    ...
    def _find_prev_non_blank_block(current_block):
        previous_block = (
            current_block.previous() if current_block.blockNumber() else None
        )
...
def make_python_patterns(additional_keywords=[], additional_builtins=[]):
    ...
    uf_sq3string = r"(\b(%s))?'''[^'\\]*((\\.|'(?!''))[^'\\]*)*(\\)?(?!''')$" % prefix
    uf_dq3string = r'(\b(%s))?"""[^"\\]*((\\.|"(?!""))[^"\\]*)*(\\)?(?!""")$' % prefix
...
def make_ipython_patterns(additional_keywords=[], additional_builtins=[]):
    return (
        make_python_patterns(additional_keywords, additional_builtins)
        + r"|^\s*%%?(?P<magic>[^\s]*)"
    )
...
class PythonSH(BaseSH):
    ...
    def highlight_match(
        self, text, match, key, value, offset, state, import_stmt, oedata
    ):
            ...
            if key == "comment":
                ...
                if value in ("def", "class"):
                    ...
                    if match1:
                        ...
                        oedata.fold_level = qstring_length(text) - qstring_length(
                            text.lstrip()
                        )
            ...
            for key, value in list(match.groupdict().items()):
                if value:
                    state, import_stmt, oedata = self.highlight_match(
                        text, match, key, value, offset, state, import_stmt, oedata
                    )
...
class CppSH(BaseSH):
    ...
    def highlight_block(self, text):
        ...
        text = to_text_string(text)
        inside_comment = (
            tbh.get_state(self.currentBlock().previous()) == self.INSIDE_COMMENT
        )
        self.setFormat(
            0,
            qstring_length(text),
            ...
            for key, value in list(match.groupdict().items()):
                if value:
                    ...
                    if key == "comment_start":
                        inside_comment = True
                        self.setFormat(
                            start, qstring_length(text) - start, self.formats["comment"]
                        )
                ...
                if value:
                    ...
                    if previous_state == self.COMMENT:
                        if key == "multiline_comment_end":
                            ...
                            self.setFormat(
                                end, qstring_length(text), self.formats["normal"]
                            )
                        else:
                            tbh.set_state(self.currentBlock(), self.COMMENT)
                            self.setFormat(
                                0, qstring_length(text), self.formats["comment"]
                            )
                    else:
                        if key == "multiline_comment_start":
                            tbh.set_state(self.currentBlock(), self.COMMENT)
                            self.setFormat(
                                start, qstring_length(text), self.formats["comment"]
                            )
                        else:
                            tbh.set_state(self.currentBlock(), self.NORMAL)
                            try:
                ...
                if value:
                    ...
                    if previous_state == self.CODE:
                        if key == "code":
                            # Change to normal
                            self.setFormat(
                                0, qstring_length(text), self.formats["normal"]
                            )
                        ...
                        if key == "code":
                            # Change to code
                            self.setFormat(
                                0, qstring_length(text), self.formats["code"]
                            )
    ...
    tokens = {
        "root": [
            (
                ...
                bygroups(Comment.Preproc, Number.Integer),
                "message",
            ),
            (r'"(.*?)"|\'(.*?)\'', String),
            (r"(\d)", Number.Integer),
        ...
        "message": [
            (
                ...
                bygroups(Text, Number, Text, Name.Builtin),
                "#pop",
            ),
            (
                r"(\s-)(\sINFO\w*)(\s-)(\s*[\d\w]+([.]?[\d\w]+)+\s*)",
                bygroups(Generic.Heading, Text, Text, Name.Builtin),
                "#pop",
            ),
