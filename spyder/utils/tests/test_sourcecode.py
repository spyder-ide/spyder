...
def test_get_identifiers():
    code = "import functools\nfunctools.partial"
    assert set(sourcecode.get_identifiers(code)) == set(
...
def test_differentiate_prefix():
    ...
    assert (
        sourcecode.differentiate_prefix(path_components0, path_components1)
        == diff_path0
    )
    assert (
        sourcecode.differentiate_prefix(path_components1, path_components0)
        == diff_path1
    )
...
def test_get_same_name_files():
    ...
    if not os.name == "nt":
        ...
        files_path_list.append(fname1)
        same_name_files = [
            ["c:", "", "documents", "test", "test.py"],
            ["c:", "", "documents", "projects", "test", "test.py"],
        ]
        assert (
            sourcecode.get_same_name_files(files_path_list, "test.py")
            == same_name_files
        )
