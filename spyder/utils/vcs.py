...
SUPPORTED = [
    {
        ...
        "rootdir": ".hg",
        "actions": dict(
            commit=(("thg", ["commit"]), ("hgtk", ["commit"])),
            browse=(("thg", ["log"]), ("hgtk", ["log"])),
        ),
    },
    {
        "name": "Git",
...
def get_git_revision(repopath):
    ...
    try:
        ...
        commit = programs.run_program(
            git, ["rev-parse", "--short", "HEAD"], cwd=repopath
        ).communicate()
        ...
        # Branch
        branches = programs.run_program(git, ["branch"], cwd=repopath).communicate()
        branches = branches[0]
        if PY3:
