...
class GithubBackend(BaseBackend):
    ...
    def __init__(self, gh_owner, gh_repo, formatter=None, parent_widget=None):
        ...
        super(GithubBackend, self).__init__(
            formatter,
            "Submit on github",
            "Submit the issue on our issue tracker on github",
            None,
    ...
    def send_report(self, title, body, application_log=None):
        ...
        try:
            ...
            _logger().warning(
                "Failed to send bug report on Github. " "response=%r", e.response
            )
            # invalid credentials
    ...
    def get_user_credentials(self):
        ...
        if not running_under_pytest():
            credentials = DlgGitHubLogin.login(
                self.parent_widget, token, remember_token
            )

            if credentials["token"] and valid_py_os:
                self._store_token(credentials["token"], credentials["remember_token"])
                CONF.set(
                    "main", "report_error/remember_token", credentials["remember_token"]
                )
        else:
