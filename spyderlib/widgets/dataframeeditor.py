# -*- coding: utf-8 -*-
"""
Created on Sun Mar 23 15:16:04 2014
...
from spyderlib.qt.QtGui import (
        QApplication, QDialog, QVBoxLayout, QTableView, QWidget, QColor, QGridLayout, QDialogButtonBox, QHBoxLayout)
...
class DataFrameModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            if index.column()==0:
                ...
                return to_qvariant(str(self.df.ix[index.row(), index.column()-1]))
    ...
    def setData(self, index, value, role):
        ...
        if isinstance(self.df.ix[index.row(), index.column()-1],(long, int, int64, float,unicode)):
                try:
                    value = float(value)
                except ValueError:
                    value = unicode(value)
...
def test():
    ...
    df1 = DataFrame([[1,'test'],[1,'test'],[1,'test'],[1,'test']],index=['a','b',nan,nan ],columns=['a','b'])
    out=test_edit(df1)
    print ("out:", out)
    return out
