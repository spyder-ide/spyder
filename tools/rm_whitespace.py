...
def git_files(file_types):
    ...
    files_final = set()
    git_commands = {
        "staged": ("git", "diff", "--name-only", "--cached", "--diff-filter=d", "-z"),
...
def filter_files(
    file_paths, include_extensions=INCLUDE_EXTENSIONS, exclude_paths=EXCLUDE_PATHS
):
...
def get_github_prs(repo_path=None):
    ...
    if repo_path is None:
        ...
        if not status_output.stdout:
            ...
            if not status_output.stdout:
                raise ValueError(
                    "Repo path not provided and could not be "
                    "determined via git remote origin/upstream."
                )
        repo_path = "/".join(status_output.stdout.split(":")[1].split("/")[-2:]).split(
            "."
        )[0]
    ...
    open_prs = [
        pr
        for pr in open_prs
        if datetime.datetime.strptime(pr["created_at"], "%Y-%m-%dT%H:%M:%SZ")
        >= PR_DATE_CUTOFF
    ]
...
def get_pr_merge_conflicts(prs_tocheck, branch_to_compare="master"):
    ...
    for pr in prs_tocheck:
        ...
        subprocess.run(
            ["git", "merge", "--no-commit", "--no-ff", "pr/" + str(pr["number"])]
        )
        status_output = subprocess.run(
        ...
        if status_output.stdout:
            merge_conflicts = {
                file_data.split("\t")[-1]
                for file_data in status_output.stdout.strip("\0").split("\0")
            }
        else:
            merge_conflicts = set()
...
def handle_whitespace(
    file_paths, whitespace_type="all", warn_only=False, verbose=False
):
    ...
    if verbose:
        print(
            "{action} {ws_type} whitespace in the following files:".format(
                action=("Detected" if warn_only else "Removed"), ws_type=whitespace_type
            )
        )
    found_files = {}
    ...
    for file_path in file_paths:
        try:
            ...
            warnings.warn(
                "{e_type} reading file {fname}: {e_msg}".format(
        ...
        if n_subs:
            ...
            if not warn_only:
                try:
                    with open(
                        file_path, mode="w", encoding="utf-8", newline=newline_char
                    ) as file:
    ...
    if verbose:
        if found_files:
            print(
                "{n_ws} instances in {n_files} file(s)".format(
                    n_ws=sum(found_files.values()), n_files=len(found_files)
                )
            )
        else:
...
def handle_whitespace_files(
    file_types, whitespace_type="all", warn_only=False, verbose=False, check_prs=False
):
    ...
    if check_prs:
        ...
        handle_whitespace(
            files_toprocess, whitespace_type=whitespace_type, warn_only=False, verbose=0
        )
        ...
        for pr in open_prs:
            ...
            if whitespace_conflicts:
                ...
                if verbose >= 2:
                    print(
                        "PR #{pr_num} has conflicts {conf}".format(
                            pr_num=pr["number"], conf=whitespace_conflicts
                        )
                    )
        if verbose:
            print(
                "Total conflicts: {num_conf} in {num_prs} PRs".format(
...
def generate_arg_parser():
    ...
    arg_parser.add_argument(
        ...
        default="both",
        help=(
            "Type of whitespace to remove. 'blank' for just whitespace on "
    ...
    arg_parser.add_argument(
        ...
        nargs="*",
        help=(
            "Type(s) of files to process, as determined by git. "
    ...
    arg_parser.add_argument(
        ...
        action="store_true",
        help=(
            "If passed, will only warn (and exit with a non-zero status) "
    ...
    arg_parser.add_argument(
        ...
        action="count",
        help=(
            "How much detail to print about the files converted. "
    ...
    arg_parser.add_argument(
        ...
        action="store_true",
        help=(
            "If passed, will check the changes to see if they cause "
